...
export async function POST(req: NextRequest) {
  ...
  try {
    const { programacion_id, rows } = await req.json();
    if (!programacion_id) throw new Error('programacion_id requerido');
    ...
    const result = await transaction(async (client) => {
      ...
      for (const r of rows) {
        ...
        if (!curso) { console.log('Curso no encontrado:', codigoCurso); continue; }

        const docente = mapDocentes[docenteDni];
        if (!docente) { console.log('Docente no encontrado:', docenteDni); continue; }
