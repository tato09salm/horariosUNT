...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Verify programacion exists
    const prog = await queryOne(`SELECT id, nombre FROM programaciones WHERE id = $1`, [id]);
    if (!prog) {
      console.error(`[programacion-cursos] No programacion found for id=${id}`);
      return NextResponse.json({ error: 'Programación no encontrada' }, { status: 404 });
    }

    // Direct query instead of getProgramacionCursosData
