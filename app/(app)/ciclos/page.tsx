...
export default function CiclosPage() {
  ...
  const cargar = useCallback(() => {
    ...
    fetch(`/api/ciclos?${params}`)
      .then(r => r.json())
      .then(data => {
        setCiclos(data.data || data || []);
        setTotal(data.total || (data.data?.length || data.length || 0));
      })
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>No se encontraron ciclos</td></tr>
              ) : (
                ciclos.map((c, i) => (
                  <tr key={c.id}>
          ...
          <div className="modal" style={{ maxWidth: '400px' }}>
            ...
            <div className="modal-body" style={{ paddingTop: '16px' }}>
              <p style={{ margin: 0, color: '#64748b', lineHeight: '1.5' }}>
                ¿Estás seguro que deseas eliminar el ciclo <strong>{cicloAEliminar?.nombre}</strong>?
            ...
            <div className="modal-footer" style={{ borderTop: 'none', paddingTop: 0, marginTop: '8px' }}>
              ...
              <button className="btn-danger" onClick={eliminarCiclo} disabled={saving}>
