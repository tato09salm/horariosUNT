...
export default function CursosPage() {
  ...
  const cargar = useCallback(() => {
    ...
    fetch(`/api/cursos?${q}`).then(r=>r.json()).then(d=>{
      setCursos(d.data||[]);
      setTotal(d.total||0);
  ...
  async function guardar() {
    ...
    try {
      ...
      setMsg({type:'success',text:`Curso ${form.id ? 'actualizado' : 'creado'} correctamente`});
  ...
  async function confirmarToggle() {
    ...
    setSaving(true);
    try {
      const nuevoEstado = !cursoAToggle.activo;
      ...
      const res = await fetch(`/api/cursos/${cursoAToggle.id}`, {
        ...
        body: JSON.stringify({ ...c, activo: nuevoEstado })
      });
      ...
      setMsg({type:'error',text:'Error al cambiar estado del curso'});
    } finally {
      setSaving(false);
    }
  }
  ...
  async function generarReporte() {
    ...
    try {
      ...
      const tableData = todosLosCursos.map((c: Curso, i: number) => [
        i + 1,
    ...
    <div className="page-container">
      ...
      <div className="card" style={{marginBottom:'16px',padding:'16px'}}>
        <div className="filters-grid">
          <input className="form-input" placeholder="Buscar por código o nombre..." value={buscar} onChange={e=>setBuscar(e.target.value)} />
          <div className="filters-group">
            <select className="form-input" style={{width:'auto'}} value={filtroCiclo} onChange={e => setFiltroCiclo(e.target.value)}>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{maxWidth:'400px'}}>
            <div className="modal-header" style={{borderBottom:'none',paddingBottom:0}}>
              <div style={{display:'flex',alignItems:'center',gap:'12px',color: cursoAToggle?.activo ? '#dc2626' : '#2563eb'}}>
                ...
                <h2 style={{fontSize:'18px',fontWeight:'600',margin:0}}>
                  ¿{cursoAToggle?.activo ? 'Desactivar' : 'Activar'} curso?
                </h2>
              </div>
            </div>
            <div className="modal-body" style={{paddingTop:'16px'}}>
              <p style={{margin:0,color:'#64748b',lineHeight:'1.5'}}>
                ¿Estás seguro que deseas {cursoAToggle?.activo ? 'desactivar' : 'activar'} el curso <strong>{cursoAToggle?.nombre}</strong>? 
                {cursoAToggle?.activo 
            ...
            <div className="modal-footer" style={{borderTop:'none',paddingTop:0,marginTop:'8px'}}>
              <button className="btn-secondary" onClick={() => {setShowConfirm(false); setCursoAToggle(null);}}>
                Cancelar
              </button>
