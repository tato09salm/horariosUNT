...
BEGIN
    ...
    FOR doc IN SELECT id, dni FROM docentes WHERE activo = true LOOP
        ...
        ELSE
            ...
            insertados := insertados + insert_disp_docente_rango(p_programacion_id, doc.id, 'martes', 8, 12, 1);
            insertados := insertados + insert_disp_docente_rango(p_programacion_id, doc.id, 'jueves', 8, 12, 2);
        END IF;
    END LOOP;
    ...
    FOR doc IN
        SELECT d.id,
               COALESCE(SUM(
                 ...
                 + pc.horas_laboratorio * GREATEST(COALESCE(cu.cantidad_labs, 1), 1)
               ), 0) + 1 AS requeridas
        FROM docentes d
        LEFT JOIN programacion_cursos pc ON pc.docente_id = d.id AND pc.programacion_id = p_programacion_id
        LEFT JOIN cursos cu ON cu.id = pc.curso_id
        WHERE d.activo = true
        GROUP BY d.id
        HAVING COALESCE(SUM(
                 pc.horas_teoria + pc.horas_practica
    ...
    LOOP
        ...
        IF horas_disp < doc.requeridas THEN
            ...
            insertados := insertados + insert_disp_docente_rango(p_programacion_id, doc.id, 'jueves', 1, 6, 1);
            insertados := insertados + insert_disp_docente_rango(p_programacion_id, doc.id, 'viernes', 1, 6, 1);
            insertados := insertados + insert_disp_docente_rango(p_programacion_id, doc.id, 'miercoles', 1, 6, 2);
        END IF;
    END LOOP;
